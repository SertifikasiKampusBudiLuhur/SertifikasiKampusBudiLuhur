    ...
    <div className="min-h-screen bg-white">
      ...
      <nav className="sticky top-0 z-50 bg-white/95 backdrop-blur-sm border-b border-slate-100 shadow-sm">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-6 text-sm font-medium text-slate-600">
            ...
            <a href="#jadwal" className="hover:text-blue-600 transition-colors">Jadwal</a>
            <a href="#fitur" className="hover:text-blue-600 transition-colors">Fitur</a>
          </div>
          <div className="flex items-center gap-2">
            <Link href="/login" className="text-sm font-semibold text-slate-700 hover:text-blue-600 transition-colors px-4 py-2 rounded-lg hover:bg-slate-50">
              Masuk
            </Link>
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-slate-900 via-blue-950 to-slate-900 text-white">
        ...
        <div className="relative max-w-6xl mx-auto px-6 py-24 text-center">
          <span className="inline-flex items-center gap-2 bg-blue-500/15 text-blue-300 text-sm font-medium px-4 py-1.5 rounded-full mb-8 border border-blue-500/25">
            <span className="w-1.5 h-1.5 bg-blue-400 rounded-full animate-pulse" />
            Sistem Sertifikasi Terintegrasi Pembayaran Otomatis
          </span>
          ...
          <p className="text-lg text-slate-300 max-w-2xl mx-auto mb-10 leading-relaxed">
            Daftar program sertifikasi kampus dalam hitungan menit. Bayar otomatis via QRIS, VA, atau e-wallet —
            tanpa upload bukti transfer, tanpa repot.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center mb-16">
            <Link href="/register" className="bg-blue-500 hover:bg-blue-400 text-white font-bold px-8 py-4 rounded-xl transition-all shadow-lg shadow-blue-900/40 text-base">
              Mulai Sekarang — Gratis
            </Link>
            <a href="#carousel" className="bg-white/8 hover:bg-white/15 text-white font-semibold px-8 py-4 rounded-xl transition-colors border border-white/15 text-base">
              Lihat Program Tersedia
            </a>
          </div>

          {/* Stats */}
          <div className="grid grid-cols-3 gap-6 max-w-md mx-auto pt-8 border-t border-white/10">
            {[
          ...
          <div className="max-w-5xl mx-auto">
            <div className="flex items-center justify-between mb-6">
              <div>
                ...
                <p className="text-slate-500 text-sm mt-1">Program sertifikasi yang sedang dibuka</p>
              </div>
      ...
      <section id="program" className="py-20 px-6 bg-white">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-12">
            ...
            <p className="text-slate-500 mt-3 text-lg">Pilih program yang sesuai dengan minat dan kebutuhanmu</p>
          </div>
          ...
          {!programs?.length ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {programs.map((program: CertificationProgram) => {
                ...
                return (
                  <div key={program.id} className="group bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden flex flex-col hover:shadow-lg hover:border-blue-200 transition-all duration-200">
                    {program.banner_url ? (
                      <div className="overflow-hidden h-48 relative">
                        <img src={program.banner_url} alt={program.nama}
                          className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                        {/* Session badge on image */}
                    ...
                    <div className="p-5 flex flex-col flex-1">
                      ...
                      <div className="mt-auto flex items-center justify-between pt-4 border-t border-slate-100">
                        ...
                        <Link href="/login" className="flex items-center gap-1.5 text-sm font-semibold bg-blue-600 hover:bg-blue-700 text-white px-4 py-2.5 rounded-xl transition-colors">
      ...
      <section id="jadwal" className="py-20 px-6 bg-slate-50">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-12">
            <h2 className="text-3xl md:text-4xl font-bold text-slate-900">Jadwal Mendatang</h2>
            <p className="text-slate-500 mt-3 text-lg">Tandai kalendermu sebelum kuota habis</p>
          </div>
          {!programs?.length ? (
            <p className="text-center text-slate-400">Belum ada jadwal tersedia</p>
          ) : (
            <div className="space-y-3">
              {programs.slice(0, 5).map((program: CertificationProgram) => {
                ...
                const sesi = SESI_BADGE[program.tipe_sesi] ?? SESI_BADGE.offline
                return (
                  <div key={program.id} className="group flex items-center gap-5 p-4 rounded-2xl border border-slate-200 bg-white hover:border-blue-200 hover:shadow-sm transition-all">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap items-center gap-2 mt-1.5">
                        ...
                        <span className="flex items-center gap-1 text-xs text-slate-500">
                          <Users size={10} /> {penuh ? 'Kuota penuh' : `${sisa} kursi`}
                        </span>
                      </div>
                    </div>
          ...
          <div className="text-center mt-8">
            <Link href="/login" className="inline-flex items-center gap-2 text-sm font-semibold text-blue-600 hover:text-blue-700 transition-colors border border-blue-200 hover:border-blue-400 px-5 py-2.5 rounded-xl hover:bg-blue-50 transition-all">
      ...
      <section id="fitur" className="py-20 px-6 bg-gradient-to-br from-slate-900 via-blue-950 to-slate-900 text-white">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-14">
            <h2 className="text-3xl md:text-4xl font-bold">Kenapa SertifikasiKampus?</h2>
            <p className="text-slate-400 mt-3 text-lg">Sistem modern yang bikin proses sertifikasi jadi efisien</p>
          </div>
            ...
            {[
              {
                ...
                desc: 'Bayar via VA, QRIS, atau e-wallet. Status langsung terupdate otomatis — admin tidak perlu verifikasi manual.',
              },
              {
                icon: <Shield size={26} />,
                color: 'bg-emerald-500',
                title: 'Verifikasi Terstruktur',
                desc: 'Admin memverifikasi kelayakan data peserta sebelum pendaftaran dikonfirmasi secara resmi dan terstruktur.',
              },
              {
                icon: <CheckCircle size={26} />,
                color: 'bg-purple-500',
                title: 'Tracking Real-time',
                desc: 'Pantau setiap tahap pendaftaranmu — dari pembayaran hingga konfirmasi akhir — kapanpun dan dimanapun.',
              },
            ].map(f => (
          ...
          <div className="mt-16 text-center bg-white/5 border border-white/10 rounded-2xl p-10">
            <h3 className="text-2xl font-bold text-white mb-3">Siap mulai sertifikasimu?</h3>
            <p className="text-slate-400 mb-8">Buat akun gratis dan daftar program yang kamu inginkan sekarang.</p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Link href="/register" className="bg-blue-500 hover:bg-blue-400 text-white font-bold px-8 py-4 rounded-xl transition-colors text-base shadow-lg shadow-blue-900/30">
                Buat Akun Gratis
              </Link>
              <Link href="/login" className="bg-white/10 hover:bg-white/20 text-white font-semibold px-8 py-4 rounded-xl transition-colors border border-white/15 text-base">
                Sudah Punya Akun
              </Link>
            </div>
          </div>
      ...
      {/* Footer */}
      <footer className="bg-slate-950 text-slate-500 py-10 px-6 border-t border-slate-800">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <p className="text-sm text-center">
            © {new Date().getFullYear()} Sistem Registrasi Sertifikasi Kampus. All rights reserved.
          </p>
